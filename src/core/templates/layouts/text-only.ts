...
export const textOnly: LayoutTemplate = {
  ...
  build({ text, pageSize: { width: W, height: H }, palette, fontId, animate }) {
    ...
    const fontSize = fitFontSize(text, box, { min: 20, max: Math.round(H * 0.09), lineHeight: 1.25 });
    const caption = createTextElement(text, box, {
      style: { fontFamily: fontId, fontSize, color: palette.text, align: 'center', verticalAlign: 'middle', lineHeight: 1.25, fontWeight: 600 },
    });
    return {
