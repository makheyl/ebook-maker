...
export function ExportDialog({ open, onOpenChange }: { open: boolean; onOpenChange: (o: boolean) => void }) {
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-lg">{open && <ExportForm onDone={() => onOpenChange(false)} />}</DialogContent>
    </Dialog>
  );
}
...
function ExportForm({ onDone }: { onDone: () => void }) {
  ...
  const [estimate, setEstimate] = useState<{ key: string; value: SizeEstimate | null }>({ key: '', value: null });
  ...
  const setSettings = (patch: Partial<typeof project.exportSettings>) =>
    docStore.change((d) => void Object.assign(d.exportSettings, patch), { label: 'Export settings' });
  ...
  return (
    <>
      <DialogHeader>
        ...
        <DialogDescription>
          Readers need nothing installed — no account, no internet. Animations and page turns work exactly
          like the preview.
        </DialogDescription>
      </DialogHeader>
            ...
            >
              ...
              <span>
                <span className="block text-sm font-medium">
                  {f.title}
                  {f.id === 'html' && <span className="ml-2 text-xs font-normal text-muted-foreground">Recommended</span>}
                </span>
      ...
      <label className="flex items-center justify-between gap-3 rounded-lg border p-3 text-sm">
        <span>
          ...
          <span className="block text-xs text-muted-foreground">A small label in the corner of the reader.</span>
        </span>
        <Switch checked={showBadge} onCheckedChange={(v) => setSettings({ showBadge: v })} aria-label="Show badge" />
      </label>
      ...
      {tooBig && (
        <p role="alert" className="flex gap-2 rounded-lg bg-amber-500/10 p-3 text-xs text-amber-700 dark:text-amber-300">
          <AlertTriangle className="size-4 shrink-0" />
          This book is over 25 MB and may open slowly, especially on phones.
          {format === 'html' ? ' The ZIP format avoids inlining overhead.' : ' Try using fewer or smaller images.'}
        </p>
      )}
      <DialogFooter>
