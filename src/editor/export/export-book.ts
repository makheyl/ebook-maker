...
export async function exportBook(
  ...
  const assets = await assetRepo.getMany(exportedAssetIds(project));
  const inputs = {
    project,
    playerJs: player.js,
...
export async function estimateExport(
  ...
  return estimateSize({
    ...
    musicBytes: [...music.values()].map((a) => a.blob.size),
